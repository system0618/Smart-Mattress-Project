...
- **气囊状态可视化**：按肩/背/腰/臀/大腿 × 左/中/右划分 15 个虚拟气囊分区（与身体部位划分一致），
  按“低压接触区充气支撑、高压区放气减压”逻辑自动“充气/放气/保持”，并高亮对应传感器点。
...
- **气囊-传感器真实对应关系**：当前 15 个气囊分区（肩/背/腰/臀/大腿 × 左/中/右）是演示用配置，位于 `js/config.js` 的 `airbagZones`。待依据《气囊-传感器 标注 布置图20240624.pdf》确认后只替换该配置即可。
